...
describe("PingPong", function () {
  beforeEach(async function () {
    ...
    // use this chainId
    this.chainIdSrc = 10002;
    this.chainIdDst = 10009;
    ...
    this.pingPongA = await PingPong.deploy(
      this.layerZeroEndpointMockSrc.address
    );
    this.pingPongB = await PingPong.deploy(
  ...
  it("increment the counter of the destination PingPong when paused should revert", async function () {
    await expect(
      this.pingPongA.ping(this.chainIdDst, this.pingPongB.address, 0)
